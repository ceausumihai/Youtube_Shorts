...
# Allow the macOS GUI to provide an absolute FFmpeg path when launched from Finder.
...
def cut_and_format_clip(
    ...
    final_path = output_dir / (output_filename or f"{slug}.mp4")
...
def main():
    ...
    parser.add_argument("--output-filename", default=None, help="Exact output filename for a single clip")
    ...
    results = []
    for clip in clips:
        final_path = cut_and_format_clip(
            video_path,
            clip,
            output_dir,
            output_filename=args.output_filename if len(clips) == 1 else None,
        )
